...
void Grupa::DodajZawodnika() {
    if (IloscZawodnikow < maxIloscZawodnikow) {
        zawodnicy.push_back(InicjalizujZawodnika()); //dodaje nowy element na koñcu kontenera vector
        ++IloscZawodnikow;
...
void Grupa::WyswietlWszystkichZawodnikow() {
    if (zawodnicy.empty()) { //sprawdza, czy ³ancuch znaków jest pusty
...
void Grupa::Edytuj(const Zawodnik& nowyZawodnik) {
    if (zawodnicy.empty()) { //sprawdza, czy lancuch znakow jest pusty
        cout << "Blad." << endl;
        return;
    }
...
void Grupa::EdytujDaneZawodnika(Grupa& Volley) {
    if (IloscZawodnikow > 0) {
        ...
        string delimiter = " "; //ogranicznik
        size_t pos = 0;
        string imie, nazwisko, druzyna, dzien_, miesiac_, rok_;

        pos = input.find(delimiter); //szuka wyst¹pienia elementu
        imie = input.substr(0, pos); //zwraca nowy string
        input.erase(0, pos + delimiter.length()); //usuwa element z kontenera 
        ...
        try {
            dzien = stoi(dzien_); //zamienia na int
            miesiac = stoi(miesiac_);
            rok = stoi(rok_);
        }
        catch (const exception& e) { //wiadomosc o bledzie
...
void Grupa::UsunLosowoZawodnika() {
    ...
    if (nullptr != zawodnicy[losowyIndex]) {
        delete zawodnicy[losowyIndex];
        zawodnicy.erase(zawodnicy.begin() + losowyIndex); //metoda usuwa element z kontenera 
        --IloscZawodnikow;

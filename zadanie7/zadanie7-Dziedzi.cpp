...
void SalaGimnastyczna::ZaplanujTrening() {
	...
	trening.resize(liczba_cwiczen); //ustawia nowy rozmiar kontenera vector

	string tymczasowy;
	size_t pozycja = 0;
	int index = 0;

	while ((pozycja = lista_cwiczen.find(',')) != string::npos && index < liczba_cwiczen) { // szuka wyst¹pienia elementu
		tymczasowy = lista_cwiczen.substr(0, pozycja); //zwraca nowy string
		trening[index++] = tymczasowy;
		lista_cwiczen.erase(0, pozycja + 1); //metoda usuwa element z kontenera 
	}
...
void Kantorek::DodajPilke(const string& pilka) {
	pilki.push_back(pilka); //dodaje nowy element na koñcu kontenera vector
	ilosc_pilek++;
}
...
void Sklep::DodajKoszulke() {
	...
	koszulki.push_back(new Koszulka(wpisany_nazw_w_druzynie, wpisane_pseudo_zawodnika, aktualny_stan_zamowienia)); //dodaje nowy element na koñcu kontenera vector
	iloscKoszulek++;
}

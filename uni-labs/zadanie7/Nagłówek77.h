...
public:
	...
	Sklep(int& _numer_grupy, const int& _iloscKoszulek, const vector<Koszulka*>& _koszulki)
		...
		for (int i = 1; i <= iloscKontenerow; i++) {
			kontenery.push_back(new Kontener(i)); //dodaje nowy element na koñcu kontenera vector
		}
	}
	~Sklep() {
...
public:
	...
	int& operator[](int index) { // przeciazenie [] operator
		if (index < 0 || index >= 20) {
			cout << "Blad." << std::endl;
			// Returning a reference to a temporary value is not recommended, but for the sake of staying in character:
			static int n = 0;
			return n;
		}
...
public:
	...
	/*
	Zawodnik(int id, size_t& _iloscId, Id**& _id_s, const string& _imie, const string& _nazwisko, const string& _druzyna, int _dzien, int _miesiac, int _rok) {
		ids.push_back(Id(id));
		iloscId++;
	}*/
